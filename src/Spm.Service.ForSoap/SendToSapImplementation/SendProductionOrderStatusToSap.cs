...
{
    public interface ISendProductionOrderStatusToSap : ISendSoapToSap<ProductionOrderStatusSapCommand> { }

    public class SendProductionOrderStatusToSap : SetupForCommunication, ISendProductionOrderStatusToSap
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(SendProductionOrderStatusToSap));
        private readonly IProductionOrderMesageMap _productionOrderMessageMap;
        private readonly IDuplicateSetupInSap _duplicateSetupInSap;
        private readonly ISerializeMessagesToFile _messagesToFile;
        private ZPP_CHNG_OB_Async_MIClient _client;

        public SendProductionOrderStatusToSap(IProductionOrderMesageMap productionOrderMessageMap,
            IDuplicateSetupInSap duplicateSetupInSap,
            ISerializeMessagesToFile messagesToFile)
        {
            _productionOrderMessageMap = productionOrderMessageMap;
            _duplicateSetupInSap = duplicateSetupInSap;
            _messagesToFile = messagesToFile;
        }

        public void SendSoapMessageToSap(ProductionOrderStatusSapCommand message)
        {
            try
            {
                SettingUpClient();
                ...
                var soapMessage = _productionOrderMessageMap.MakeSoapMessage(message);
                ...
                _client.ZPP_CHNG_OB_Async_MI(soapMessage);

                Logger.Info("Writing copy of message to file.");
                _messagesToFile.SaveMessageToXmlFile(soapMessage, message.ProductionOrderId, Constants.ProductionOrderStatusOutputFolder, Shared.Constants.ProductionOrderStatusFileCount);
        ...
        internal override void SettingUpClient()
        {
            ...
            _client = new ZPP_CHNG_OB_Async_MIClient();
        }
    }
}