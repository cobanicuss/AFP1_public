...
{
    public class GoodsReversalHandler : IHandleMessages<GoodsReversalCommand>
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(GoodsReversalHandler));
        ...
        private const string GoodsIdent = Constants.GoodsReversalType;
        private const string GoodsType = Shared.Constants.GoodsReversalType;

        public GoodsReversalHandler(
        ...
        public void Handle(GoodsReversalCommand message)
        {
            const float leg = 1.0F;

            _messageType = typeof(GoodsReversalCommand).FullName;
            _endpoint = typeof(GoodsReversalHandler).FullName;
            ...
            _fileWatcherRepository.InsertGoodsReversalData(goodsSplittingDto.MappedDataDtoList, message.FileName, GoodsType);
        ...
        private void SendAuditTrailMessage(string sagaReferenceId, string goodsReceiptId, int messageItemCount, float leg)
        {
            var productionOrderStatusAuditCommand = new GoodsReceiptAuditCommand
            {
                ...
                Type = Shared.Constants.GoodsReversalType,
                Leg = leg
            };
